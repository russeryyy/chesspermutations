...
export function looksLikeFenInput(value: string): boolean {
  const placement = value.trim().split(/\s+/, 1)[0] ?? '';
  return placement.split('/').length === 8;
}
...
export async function appendMove(
  ...
  mainline = false,
  annotations?: NodeAnnotations,
): Promise<{ study: StudyModel; node: GameNode }> {
  const parent = study.nodes[parentId];
  if (!parent) throw new Error('Parent position is unavailable.');
  const existing = parent.children
    .map((id) => study.nodes[id])
    .find((node) => node.uci === uci);
  if (existing) {
    const promoted =
      mainline && !existing.mainline
        ? { ...existing, mainline: true }
        : existing;
    const mainlineIds =
      mainline && parent.id === study.mainline[study.mainline.length - 1]
    ...
    return {
      study: {
        ...
        nodes:
          promoted === existing
            ? study.nodes
            : { ...study.nodes, [promoted.id]: promoted },
      },
      node: promoted,
    };
  }
  ...
  const mainlineIds =
    mainline && parent.id === study.mainline[study.mainline.length - 1]
      ? [...study.mainline, node.id]
      : study.mainline;
  return {
    study: { ...study, nodes, activeId: node.id, mainline: mainlineIds },
    node,
  };
}
...
export async function expandNode(
  ...
  const nodes = {
    ...
    [parent.id]: {
      ...parent,
      expanded: true,
      children: created.map((node) => node.id),
    },
  };
...
export async function buildStudyFromSan(
...
export function pgnForPath(study: StudyModel, nodeId = study.activeId): string {
  ...
  chess.setHeader(
    'Event',
    study.localTitle ||
      study.importedHeaders?.Event ||
      'chesspermutations study',
  );
