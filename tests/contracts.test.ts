...
describe('frozen chess-game:v1 identities', () => {
  it('keeps a fixed phrase and path on golden addresses', async () => {
    expect(await phraseAddress('a map with no edge')).toBe('SdticEYWTiJd3gIJ4zM1bV9rVVKodzAPYmPPOGdPduF');
    expect(await phraseAddress('  a map with no edge  ')).toBe('SdticEYWTiJd3gIJ4zM1bV9rVVKodzAPYmPPOGdPduF');
    expect(await nodeIdentity(START_FEN, ['e2e4', 'e7e5'])).toBe('hr32GXTr68MVEiJ1H9XxXtfposatrkgPtWQ1MCxQiZ2');
  });

  it('round-trips all share source types without a phrase', () => {
    const generated = readShareHash(shareHash({ kind: 'seed', address: 'abc123' }, ['e2e4'], 'black'));
    expect(generated).toEqual({ version: 1, source: { kind: 'seed', address: 'abc123' }, activePath: ['e2e4'], orientation: 'black' });
    expect(shareHash({ kind: 'fen', fen: START_FEN }, [], 'white')).not.toContain('a map with no edge');
    expect(readShareHash(shareHash({ kind: 'pgn', pgn: '1. e4 *' }, [], 'white'))?.source).toEqual({ kind: 'pgn', pgn: '1. e4 *' });
  });
});

describe('rules, paths, and positions', () => {
  it('recognizes castling, promotion, en passant, mate, stalemate, and repetition', async () => {
    const castle = await createStudy({ kind: 'fen', fen: 'r3k2r/8/8/8/8/8/8/R3K2R w KQkq - 0 1' }, 'r3k2r/8/8/8/8/8/8/R3K2R w KQkq - 0 1');
    expect((await appendMove(castle, castle.rootId, 'e1g1')).node.san).toBe('O-O');

    const promotion = await createStudy({ kind: 'fen', fen: '7k/P7/8/8/8/8/8/7K w - - 0 1' }, '7k/P7/8/8/8/8/8/7K w - - 0 1');
    expect((await appendMove(promotion, promotion.rootId, 'a7a8q')).node.san).toContain('=Q');

    const passant = await buildStudyFromSan(['e4', 'a6', 'e5', 'd5', 'exd6'], { kind: 'seed', address: 'rules' });
    expect(passant.nodes[passant.activeId].uci).toBe('e5d6');

    const mate = await buildStudyFromSan(['f3', 'e5', 'g4', 'Qh4#'], { kind: 'seed', address: 'mate' });
    expect(mate.nodes[mate.activeId].terminal).toContain('checkmate');

    const stalemate = await createStudy({ kind: 'fen', fen: '7k/5Q2/6K1/8/8/8/8/8 b - - 0 1' }, '7k/5Q2/6K1/8/8/8/8/8 b - - 0 1');
    expect(stalemate.nodes[stalemate.rootId].terminal).toContain('stalemate');

    const repetition = await buildStudyFromSan(['Nf3', 'Nf6', 'Ng1', 'Ng8', 'Nf3', 'Nf6', 'Ng1', 'Ng8'], { kind: 'seed', address: 'repeat' });
    expect(repetition.nodes[repetition.activeId].terminal).toContain('threefold');

    const fifty = await createStudy({ kind: 'fen', fen: '7k/8/8/8/8/8/8/R6K w - - 99 50' }, '7k/8/8/8/8/8/8/R6K w - - 99 50');
    expect((await appendMove(fifty, fifty.rootId, 'a1a2')).node.terminal).toContain('fifty-move');
  });

  it('keeps path identity distinct while recognizing transpositions', async () => {
    const first = await buildStudyFromSan(['e4', 'e5', 'Nf3', 'Nc6'], { kind: 'seed', address: 'one' });
    const second = await buildStudyFromSan(['Nf3', 'Nc6', 'e4', 'e5'], { kind: 'seed', address: 'two' });
    expect(first.nodes[first.activeId].id).not.toBe(second.nodes[second.activeId].id);
    expect(first.nodes[first.activeId].positionKey).toBe(second.nodes[second.activeId].positionKey);
  });

  it('lays out nodes deterministically with ply on z', async () => {
    const study = await buildStudyFromSan(['e4', 'e5', 'Nf3'], { kind: 'seed', address: 'layout' });
    const nodes = Object.values(study.nodes);
    expect(layoutGameTree(nodes)).toEqual(layoutGameTree(nodes));
    for (const point of layoutGameTree(nodes)) expect(point.z).toBe(study.nodes[point.id].ply === 0 ? 0 : -study.nodes[point.id].ply * 9);
  });
...
describe('annotated imports and adjudication', () => {
  it('preserves comments and recursive variations after atomic validation', async () => {
    const pgn = '[Event "Nested study"]\n\n1. e4 {King pawn} e5 2. Nf3 (2. Bc4 Nf6 (2... Bc5)) Nc6 $1 *';
    const study = await importAnnotatedPgn(pgn);
    expect(study.localTitle).toBe('Nested study');
    expect(Object.values(study.nodes).some((node) => node.annotations?.comment === 'King pawn')).toBe(true);
    expect(Object.values(study.nodes).filter((node) => node.ply === 3).length).toBeGreaterThan(1);
    expect(pgnForPath(study)).toBe(pgn);
  });

  it('rejects multiple games and illegal branches', async () => {
    await expect(importAnnotatedPgn('1. e4 *\n\n1. d4 *')).rejects.toThrow(/one PGN|one.*time/i);
    await expect(importAnnotatedPgn('1. e5 *')).rejects.toThrow();
  });

  it('freezes the v1 adjudication windows', () => {
    expect(adjudicate(Array(8).fill(701), 40)).toEqual({ result: '1-0', reason: 'engine adjudication · sustained +7.00' });
    expect(adjudicate(Array(20).fill(0), 80)).toEqual({ result: '1/2-1/2', reason: 'engine adjudication · sustained equality' });
    expect(adjudicate([], 320)).toEqual({ result: '1/2-1/2', reason: 'generation horizon · 320 plies' });
  });
  ...
  it('refuses oversized portable PGN links while keeping local imports possible', () => {
    ...
    expect(() => shareHash({ kind: 'pgn', pgn: noisy }, [], 'white')).toThrow(/too large/i);
  });
});
