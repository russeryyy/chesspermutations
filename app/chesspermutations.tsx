...
import {
  readCachedProbabilities,
  readCachedStudy,
  writeCachedProbabilities,
  writeCachedStudy,
} from '@/lib/chess/cache';
import {
  ENGINE_CONTRACT,
  GRAPH_NODE_LIMIT,
  type Evaluation,
...
function webMcpInput(
  ...
  if (Object.keys(value).some((key) => !allowedKeys.includes(key)))
    throw new Error('input contains unsupported properties');
  return value;
}
...
async function followSharedPath(
  study: StudyModel,
  ...
  for (const uci of path) {
    const existing = next.nodes[parentId]?.children
...
function AnalysisBoard({
  ...
  const config = useCallback(() => {
    ...
    const autoShapes = [
      ...(node.annotations?.arrows ?? []).map((arrow) => ({
        orig: arrow.from as Key,
        dest: arrow.to as Key,
        brush: colorMap[arrow.color] ?? 'green',
      })),
      ...(node.annotations?.squares ?? []).map((square) => ({
        orig: square.square as Key,
        brush: colorMap[square.color] ?? 'green',
      })),
    ];
    return {
      fen: node.fen,
      ...
      movable: {
        ...
        events: {
          after: (from: Key, to: Key) => {
            const legal = new Chess(node.fen)
              .moves({ square: from as Square, verbose: true })
              .filter((move) => move.to === to);
            const choice =
              legal.find((move) => move.promotion === 'q') ?? legal[0];
  ...
  return (
    <div
      ref={mount}
      className="cg-wrap chess-board"
      aria-label={`Interactive chess board at ply ${node.ply}`}
    />
  );
}

function probabilitiesForLines(
  study: StudyModel,
  parentId: string,
  lines: readonly AnalysisLine[],
): Record<string, WinProbability> {
  const parent = study.nodes[parentId];
  if (!parent) return {};
  const childByMove = new Map(
    parent.children.map((id) => [study.nodes[id]?.uci, study.nodes[id]]),
  );
  const turn = new Chess(parent.fen).turn();
  const entries = lines.flatMap((line) => {
    const child = childByMove.get(line.move);
    const probability = probabilityFromLine(line, turn);
    return child && probability ? [[child.id, probability] as const] : [];
  });
...
function AccessibleTree({
  ...
  const ordered = useMemo(
    () =>
      Object.values(study.nodes).sort(
        (left, right) =>
          left.ply - right.ply || left.id.localeCompare(right.id),
      ),
    [study.nodes],
  );
  return (
    <div className="tree-fallback" role="tree" aria-label="Chess move tree">
      {ordered.slice(0, GRAPH_NODE_LIMIT).map((node) => (
        <button
          key={node.id}
          role="treeitem"
          aria-current={node.id === study.activeId ? 'true' : undefined}
          aria-level={node.ply + 1}
          className={node.id === study.activeId ? 'active' : ''}
          style={{ paddingLeft: `${12 + Math.min(node.ply, 10) * 12}px` }}
          onClick={() => onSelect(node.id)}
        >
          <span>
            {node.ply ? `${node.ply}. ${node.san}` : 'Starting position'}
          </span>
          <small>
            {node.terminal ??
              (probabilities[node.id]
                ? leaderProbabilityLabel(probabilities[node.id])
                : analysisUnavailable
                  ? 'Unavailable'
                  : 'Analyzing')}
          </small>
...
export function ChessPermutations() {
  ...
  const [graphMode, setGraphMode] = useState<'3d' | 'list'>(() =>
  ...
  const backgroundAbort = useRef<AbortController | null>(null);
  const probabilityCacheReads = useRef(new Set<string>());
  ...
  const startGenerationRef = useRef<
    (base: StudyModel, follow: boolean) => void
  >(() => undefined);
  ...
  const mergeProbabilities = useCallback(
    (records: Readonly<Record<string, WinProbability>>) => {
      if (!Object.keys(records).length) return;
      setProbabilities((current) => ({ ...current, ...records }));
      void writeCachedProbabilities(records);
    },
    [],
  );
  ...
  const commitStudy = useCallback((next: StudyModel) => {
    studyRef.current = next;
    setStudy(next);
  }, []);
  const getEngine = useCallback(() => {
    if (!engine.current) engine.current = new StockfishClient();
    ...
    (message: string, type: 'info' | 'success' | 'error' = 'info') => {
      ...
      if (studyRef.current)
        toast.add({
          title: message,
          type,
          timeout: type === 'error' ? 6_000 : 3_000,
  ...
  const startGeneration = useCallback(
    (base: StudyModel, follow: boolean) => {
      ...
      void (async () => {
        ...
        try {
          while (
            ...
            generated.mainline.length <= ENGINE_CONTRACT.horizonPlies
          ) {
            ...
            setEngineStatus(
              `GENERATING · PLY ${String(parent.ply).padStart(3, '0')}`,
            );
            const lines = await getEngine().analyze(
              generated.rootFen,
              parent.path,
              {
                nodes: ENGINE_CONTRACT.generationNodes,
                signal: controller.signal,
              },
            );
            if (controller.signal.aborted || run !== generationRun.current)
              return;
            if (!lines.length)
              throw new Error('Stockfish returned no legal continuation.');
            setEngineLines(lines);
            const chosen = await chooseSeededLine(
              lines,
              address,
              parent.ply,
              parent.id,
            ...
            const parentProbability = probabilityFromLine(
              lines[0],
              position.turn(),
            );
            if (parentProbability)
              mergeProbabilities({ [parentId]: parentProbability });
            whiteScores.push(scoreAsCentipawns(whiteEvaluation));
            generated = {
              ...generated,
              nodes: {
                ...generated.nodes,
                [parentId]: { ...parent, evaluation: whiteEvaluation },
              },
            };
            generated = await expandNode(generated, parentId);
            mergeProbabilities(
              probabilitiesForLines(generated, parentId, lines),
            );
            const appended = await appendMove(
              generated,
              parentId,
              chosen.move,
              true,
            ...
            generated = {
              ...appended.study,
              activeId: follow ? appended.node.id : generated.activeId,
            };
            ...
            if (ruling)
              generated = {
                ...
                termination: ruling.reason,
                nodes: {
                  ...generated.nodes,
                  [appended.node.id]: {
                    ...generated.nodes[appended.node.id],
                    terminal: ruling.reason,
                  },
                },
              };
            const exactProbability = terminalProbability(
              generated.nodes[appended.node.id],
            );
            if (exactProbability)
              mergeProbabilities({ [appended.node.id]: exactProbability });
          ...
          if (!controller.signal.aborted) {
            ...
            notify(
              generated.termination
                ? `Generated line complete · ${generated.termination}`
                : 'Generated line cached locally.',
              'success',
            );
          }
        } catch (error) {
          if (!(error instanceof DOMException && error.name === 'AbortError')) {
            notify(
              error instanceof Error
                ? error.message
                : 'Local generation stopped unexpectedly.',
              'error',
            );
    ...
    [commitStudy, getEngine, mergeProbabilities, notify],
  );
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (shared?.source.kind === 'pgn')
          initial = await importAnnotatedPgn(shared.source.pgn);
        else if (shared?.source.kind === 'fen')
          initial = await createStudy(
            shared.source,
            shared.source.fen,
            'Shared FEN study',
          );
        else if (shared?.source.kind === 'seed')
          initial =
            (await readCachedStudy(shared.source.address)) ??
            (await createStudy(
              shared.source,
              START_FEN,
        ...
        commitStudy(initial);
        if (shared?.source.kind === 'seed' && initial.mainline.length === 1)
          startGenerationRef.current(initial, true);
      } catch (error) {
        if (!alive) return;
        setNotice(
          error instanceof Error
  ...
  useEffect(() => {
    if (!study) return;
    const rootId = study.rootId;
    const nodes = Object.values(study.nodes);
    const exact = Object.fromEntries(
      nodes.flatMap((node) => {
        const probability = terminalProbability(node);
        return probability ? [[node.id, probability] as const] : [];
      }),
    );
    queueMicrotask(() => {
      if (studyRef.current?.rootId === rootId) mergeProbabilities(exact);
    });
    const unread = nodes
      .map((node) => node.id)
      .filter((id) => !probabilityCacheReads.current.has(id));
    if (!unread.length) return;
    unread.forEach((id) => probabilityCacheReads.current.add(id));
    void readCachedProbabilities(unread).then((cached) => {
      if (studyRef.current?.rootId === rootId)
        setProbabilities((current) => ({ ...cached, ...current }));
    });
  }, [mergeProbabilities, study]);
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      if (!controller.signal.aborted) setEngineStatus('ANALYZING · 30K NODES');
    });
    void (async () => {
      try {
        const lines = await getEngine().analyze(
          currentStudy.rootFen,
          selectedNode.path,
          { signal: controller.signal },
        );
        if (
          controller.signal.aborted ||
          studyRef.current?.activeId !== selectedId
        )
          return;
        setEngineLines(lines);
        const best = lines[0];
        if (best) {
          ...
          const selectedProbability = probabilityFromLine(best, turn);
          if (selectedProbability)
            mergeProbabilities({ [selectedId]: selectedProbability });
          const current = studyRef.current;
          if (current?.nodes[selectedId])
            commitStudy({
              ...current,
              nodes: {
                ...current.nodes,
                [selectedId]: {
                  ...
                  evaluation: evaluationForWhite(best, turn),
                },
              },
            });
        }
        const latestStudy = studyRef.current;
        if (latestStudy?.nodes[selectedId])
          mergeProbabilities(
            probabilitiesForLines(latestStudy, selectedId, lines),
          );
        ...
        if (
          ...
          studyRef.current.source.kind === 'seed'
        ) {
  ...
  }, [activeId, commitStudy, getEngine, mergeProbabilities, notify]);
  ...
  useEffect(() => {
    ...
    if (
      ...
      !pageVisible ||
      generationActive.current ||
      engineStatus !== 'LOCAL · IDLE'
    )
      return;
    const parents = openedAnalysisQueue(current, probabilities);
    const parent = parents[0];
    if (!parent?.children.length) return;
    const controller = new AbortController();
    backgroundAbort.current = controller;
    const rootId = current.rootId;
    const pathKey = parent.path.join(' ');
    const timer = window.setTimeout(() => {
      void getEngine()
        .analyze(current.rootFen, parent.path, {
          nodes: ENGINE_CONTRACT.analysisNodes,
          multiPv: parent.children.length,
          signal: controller.signal,
        })
        .then((lines) => {
          const latestStudy = studyRef.current;
          const latestParent = latestStudy?.nodes[parent.id];
          if (
            controller.signal.aborted ||
            !latestStudy ||
            latestStudy.rootId !== rootId ||
            latestParent?.path.join(' ') !== pathKey
          )
            return;
          mergeProbabilities(
            probabilitiesForLines(latestStudy, parent.id, lines),
          );
        })
        .catch((error) => {
          if (!(error instanceof DOMException && error.name === 'AbortError'))
  ...
  const openSeed = useCallback(
    async (value: string) => {
      try {
        ...
        notify(
          cached
            ? 'Opened cached permanent game.'
            : 'Seed created. Streaming deterministic moves…',
          'success',
        );
        ...
        return {
          ...
          activePly: next.nodes[next.activeId].ply,
        };
      } catch (error) {
        notify(
          error instanceof Error
            ? error.message
            : 'The phrase could not be opened.',
          'error',
        );
        throw error;
      }
  ...
  const openImport = useCallback(
    async (kind: 'fen' | 'pgn', value: string) => {
      ...
      try {
        ...
        notify(
          kind === 'fen'
            ? 'FEN validated and opened.'
            : 'Annotated PGN validated and imported.',
          'success',
        );
        return {
          kind,
          title: next.localTitle ?? null,
  ...
  const submitImport = useCallback(() => {
    void openImport(importKind, importText).catch(() => undefined);
  }, [importKind, importText, openImport]);
  ...
  const copyShareLink = useCallback(async () => {
    ...
    try {
      const hash = shareHash(
        current.source,
        current.nodes[current.activeId].path,
        orientation,
      );
      ...
      notify(
        'Versioned link copied. The original phrase is not in it.',
        'success',
      );
    } catch (error) {
      notify(
  ...
  const exportPgn = useCallback(() => {
    const current = studyRef.current;
    if (!current) return;
    const blob = new Blob([pgnForPath(current)], {
  ...
  useEffect(() => {
    ...
    const register = (tool: Parameters<WebMcpContext['registerTool']>[0]) => {
      try {
        ...
        /* Unsupported or duplicate registries must not affect the visible app. */
      }
    };
    register({
      ...
      description:
        'Hash a phrase locally, open its permanent chess-game:v1 address, and begin deterministic local generation.',
      inputSchema: {
        type: 'object',
        properties: {
      ...
      execute: async (input) => {
        ...
        if (
          ...
          !phraseInput.trim() ||
          phraseInput.length > 512
        )
          throw new Error(
            'phrase must be a non-empty string of at most 512 characters',
          );
    ...
    register({
      ...
      description:
        'Validate and open one FEN position or one annotated PGN game locally, preserving legal recursive variations.',
      inputSchema: {
        type: 'object',
        properties: {
          kind: { type: 'string', enum: ['fen', 'pgn'] },
      ...
      execute: async (input) => {
        ...
        if (
          (value.kind !== 'fen' && value.kind !== 'pgn') ||
          typeof value.content !== 'string' ||
          !value.content.trim()
        )
          throw new Error(
            'kind must be fen or pgn and content must be non-empty',
          );
    ...
    register({
      ...
      description:
        'Navigate the current study to a complete legal path of UCI moves and update the board, notation, and permutation tree together.',
      inputSchema: {
        type: 'object',
        properties: {
          moves: {
      ...
      execute: async (input) => {
        ...
        if (
          ...
          throw new Error(
            'moves must be an array of at most 320 legal UCI moves',
          );
        const current = studyRef.current;
        ...
        return {
          activePly: next.nodes[next.activeId].ply,
          fen: next.nodes[next.activeId].fen,
          nodeId: next.activeId,
        };
      },
    });
    register({
      ...
      description:
        'Read the current local study address, active position, source kind, node count, and engine status without changing it.',
      inputSchema: {
        type: 'object',
        properties: {},
        additionalProperties: false,
    ...
    register({
      ...
      description:
        'Create a versioned local share link for the active source and move path without serializing an original phrase.',
      inputSchema: {
        type: 'object',
        properties: {},
        additionalProperties: false,
  ...
  const lineNodes = useMemo(
    () => (study ? nodePath(study, study.activeId) : []),
    [study],
  );
  ...
  const activeTimelineIndex = Math.max(
    0,
    timelineNodes.findIndex((node) => node.id === study?.activeId),
  );
  ...
  const evalPercent = currentEvaluation
    ? Math.max(8, Math.min(92, 50 + scoreAsCentipawns(currentEvaluation) / 20))
    : 50;

  const stepMainline = useCallback(
    (delta: number) => {
      const current = studyRef.current;
      if (!current) return;
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      const current = studyRef.current;
      if (!current) return;
      const next = current.nodes[current.activeId].children.find(
        (id) => current.nodes[id]?.mainline,
      );
      if (!next) {
        setPlaying(false);
        return;
      ...
      <main className="chesspermutations-app">
        <header className="app-header">
          ...
          <div className="header-actions">
            <a
              ...
              href="https://github.com/russeryyy/chesspermutations"
              target="_blank"
              rel="noreferrer"
            >
              Github
        ...
        >
          <TabsList>
            ...
            <TabsTrigger value="analysis">Moves</TabsTrigger>
            <TabsTrigger value="permutations">Tree</TabsTrigger>
          </TabsList>
        </Tabs>
        ...
        >
          <section
            ...
            aria-label="Chess board and timeline"
          >
            <div className="board-stage">
              <div
                className="eval-track"
                aria-label={`Evaluation ${scoreLabel(currentEvaluation)}`}
              >
                <span style={{ height: `${evalPercent}%` }} />
              </div>
              <AnalysisBoard
                node={activeNode}
                orientation={orientation}
            ...
            <div className="transport">
              ...
              <Slider
                ...
                value={Math.min(activeTimelineIndex, Math.max(1, timelineMax))}
                onValueChange={(value) => {
                  const index = typeof value === 'number' ? value : value[0];
                  const target = timelineNodes[index];
                  if (target) void selectNode(target.id);
                }}
              />
              <Button
                variant="outline"
              ...
              <span>
                {String(activeTimelineIndex).padStart(2, '0')} /{' '}
                {String(timelineMax).padStart(2, '0')}
              </span>
            </div>
            <div className="board-utilities">
              ...
              <div className="board-meta">
                <strong
                  aria-label={`Evaluation ${scoreLabel(currentEvaluation)}`}
                >
                  {scoreLabel(currentEvaluation)}
                </strong>
                {activeNode.terminal && <span>{activeNode.terminal}</span>}
              </div>
            </div>
          </section>
          ...
          <aside className="analysis-panel" aria-label="Move analysis">
            <div className="analysis-head">
              <strong
                className={
                  engineStatus.startsWith('ENGINE UNAVAILABLE')
                    ? 'engine-state engine-error'
                    : 'engine-state'
                }
              >
                <i />
            ...
            <section className="move-path">
              <header>
                <span>Line</span>
                <small>{activeNode.ply} ply</small>
              </header>
              <div className="move-list" aria-label="Selected move path">
                {lineNodes
                  .slice(1)
                  .reduce<
                    Array<{
                  ...
                  >((rows, node) => {
                    const index = Math.floor((node.ply - 1) / 2);
                    rows[index] ??= { number: index + 1 };
                    if (node.ply % 2) rows[index].white = node;
                    else rows[index].black = node;
                    return rows;
                  }, [])
                  .map((row) => (
                    <div
                      key={row.number}
                      className={
                        row.white?.id === activeNode.id ||
                        row.black?.id === activeNode.id
                          ? 'current-move'
                          : ''
                      }
                    >
                      ...
                      {row.white ? (
                        <button onClick={() => void selectNode(row.white!.id)}>
                          {row.white.san}
                        </button>
                      ) : (
                        <i />
                      ...
                      {row.black ? (
                        <button onClick={() => void selectNode(row.black!.id)}>
                          {row.black.san}
                        </button>
                      ) : (
                        <i />
            ...
            {activeNode.annotations?.comment && (
              <p className="node-comment">{activeNode.annotations.comment}</p>
            )}
            <section className="engine-lines">
              <header>
                <span>Lines</span>
                <small>{engineLines.length} PV</small>
              </header>
              <div className="analysis-scroll">
                {engineLines.slice(0, 6).map((line) => (
                  <button
                    key={`${line.multipv}-${line.move}`}
                    onClick={() => void makeMove(line.move)}
                  >
                    <strong>{lineSan(activeNode.fen, line.move)}</strong>
                    <span>{scoreLabel(line)}</span>
                    <small>{line.pv.slice(1, 5).join(' ')}</small>
                  </button>
                ))}
              </div>
            </section>
            <section className="continuations">
              ...
              <div className="analysis-scroll">
                {candidates.map((move, index) => {
                  ...
                  const analyzed = engineLines.find(
                    (line) => line.move === uci,
                  );
                  return (
                    <button key={uci} onClick={() => void makeMove(uci)}>
                      <strong>{move.san}</strong>
                      <span>{analyzed ? scoreLabel(analyzed) : 'branch'}</span>
                      <i style={{ width: `${Math.max(8, 42 - index * 3)}%` }} />
                    </button>
                  );
                })}
              </div>
            ...
            <div className="analysis-actions">
              ...
              {study.source.kind === 'seed' && (
                <Button
                  variant="ghost"
                  onClick={() => startGeneration(studyRef.current!, false)}
                >
                  <RotateCcw /> Continue
                </Button>
              )}
          ...
          <section
            className="permutations-panel"
            aria-labelledby="permutations-title"
          >
            <div className="graph-toolbar">
              <h2 id="permutations-title">Move permutations</h2>
              <div className="graph-actions">
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() =>
                    setGraphMode((value) => (value === '3d' ? 'list' : '3d'))
                  }
                >
                  <ListTree /> {graphMode === '3d' ? '2D tree' : '3D view'}
                </Button>
                <Button
                  variant="ghost"
                  size="icon-sm"
            ...
            <div className="permutations-view">
              {graphMode === '3d' ? (
                <Suspense
                  fallback={
                    <div className="graph-loading">Mapping local branches…</div>
                  }
                >
                  <PermutationsGraph
                    nodes={treeNodes}
                    ...
                    analysisUnavailable={engineStatus.startsWith(
                      'ENGINE UNAVAILABLE',
                    )}
                    onSelect={selectNode}
                  />
            ...
            <div className="graph-legend permutations-legend">
              <span>
                <i className="white-edge" />
                White move
              </span>
              <span>
                <i className="black-edge" />
                Black move
              </span>
              <span>
                <i className="wdl-halo" />W / D / L halo
              </span>
              <strong>
                {Math.min(treeNodes.length, GRAPH_NODE_LIMIT).toLocaleString()}{' '}
                / {GRAPH_NODE_LIMIT.toLocaleString()}
              </strong>
            </div>
          </section>
        </section>
        ...
        <Dialog open={importOpen} onOpenChange={setImportOpen}>
          <DialogContent className="import-dialog">
            <DialogHeader>
              <DialogTitle>Import a local study</DialogTitle>
              <DialogDescription>
                FEN and one annotated PGN are validated entirely in this
            ...
            <Tabs
              value={importKind}
              onValueChange={(value) => {
                setImportKind(value as 'fen' | 'pgn');
                setImportError('');
              }}
            >
              <TabsList>
                <TabsTrigger value="fen">FEN</TabsTrigger>
                <TabsTrigger value="pgn">Annotated PGN</TabsTrigger>
              </TabsList>
        ...
        <Dialog open={licensesOpen} onOpenChange={setLicensesOpen}>
          <DialogContent className="license-dialog">
            ...
            <div className="license-list">
              <p>
                <strong>chesspermutations</strong>
                <br />
                GPLv3 ·{' '}
                <a href="/chesspermutations-source-v1.zip" download>
                  download this build’s source
                </a>{' '}
                ·{' '}
                <a href="/SOURCE.txt" target="_blank">
                  source notice
                </a>
              </p>
              <p>
                <strong>Stockfish.js 18.0.8 lite single-thread</strong>
                <br />
                GPLv3 · 30,000 nodes · Threads 1 · Hash 16 ·{' '}
                <a
                ...
                ·{' '}
                <a href="/stockfish/COPYING.txt" target="_blank">
                  license
                </a>
              </p>
              <p>
                <strong>Chessground 10.1.1</strong>
                <br />
                GPL-3.0-or-later ·{' '}
                <a
                  href="https://github.com/lichess-org/chessground/tree/v10.1.1"
                  target="_blank"
                  rel="noreferrer"
                >
                  source
                ...
                ·{' '}
                <a href="/CHESSGROUND-LICENSE.txt" target="_blank">
                  license
                </a>
              </p>
            </div>
